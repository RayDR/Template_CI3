...
function frep_guardar(e){
   ...
   var respuesta,
       errores = '',
       datos   = {
         'preproyecto_detallada':  $('#preproyecto_detallada_id').val(),
         'mes':                  $('#mes').val(),
   ...
   if ( $('#preproyecto_detallada_id').val() == null || $("#preproyecto_detallada_id").val() == undefined )
      $('#preproyecto_detallada_id').val($(this).find('option:selected').data('preproyecto_detallada'));

   if ( !errores ){
      respuesta   = fu_json_query(
         url('Preproyectos/registrar_reporte', true, false),
         datos 
      );
      if ( respuesta.exito ){
...
function frep_reporte(e){
   var rp_Fisico           = $(this).find('option:selected').data('programado_fisico'),
       ...
       preproyecto_detallada = $(this).find('option:selected').data('preproyecto_detallada');
   ...
   $('#preproyecto_detallada_id').val(preproyecto_detallada);
}